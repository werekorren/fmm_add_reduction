...
class BilinearMatMul(nn.Module):
    def __init__(self, n, r, dtype=torch.float32):
        super().__init__()
        self.n = n
        self.r = r
        self.dtype = dtype
        
        # Input: (batch, n, n) -> (batch, n*n, 1)
        # Weights shape: (r, n*n)
        self.WA = nn.Linear(n*n, r, bias=False, dtype=dtype)
        self.WB = nn.Linear(n*n, r, bias=False, dtype=dtype)
        self.WC = nn.Linear(r, n*n, bias=False, dtype=dtype)
    ...
    def forward(self, x):
        ...
        # Flatten matrices: (batch, n, n) -> (batch, n*n)
        A_flat = A.reshape(bsz, -1)
        B_flat = B.reshape(bsz, -1)
        
        # Apply linear transformations: (batch, n*n) -> (batch, r)
        ...
        # Final transformation: (batch, r) -> (batch, n*n)
        C_flat = self.WC(products)
        
        # Reshape back: (batch, n*n) -> (batch, n, n)
        C = C_flat.reshape(bsz, self.n, self.n)
