...
net = BilinearMatMul(args.n, args.m, args.k, args.r, dtype=args.dtype).to(args.device)
...
for step in range(1, args.epochs):
    ...
    A = torch.randn(args.batch, args.n, args.m, device=args.device, dtype=args.dtype)
    B = torch.randn(args.batch, args.m, args.k, device=args.device, dtype=args.dtype)
    ...
    if step % 100 == 0:
        ...
        if ternary_start_step is None and val_loss < args.enable_ternary_at:
            ...
            print(f"Enabled ternary regularisation")
